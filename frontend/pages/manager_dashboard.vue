<template>
  <div v-if="can(`manager_dashboard_access`)">
    ...
    <v-data-table
      v-if="can(`manager_dashboard_view`)"
      :headers="headers"
      :items="data"
...
export default {
  ...
  created() {
    this.loading = true;
    this.payload.department_id = this.$auth.user.employee.department_id;
  ...
  methods: {
    ...
    getEmployeesByDepartment() {
      ...
      this.$axios
        .get(
          `/employees_by_departments/${u.employee.department.id}`,
          this.custom_options
        )
        .then(({ data }) => {
