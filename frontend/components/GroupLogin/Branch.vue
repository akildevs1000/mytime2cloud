          ...
          <v-card class="mb-5 mt-2 rounded-md" elevation="0">
            ...
            >
              <template v-slot:item.branches="{ item }">
                <span v-for="(branch, index) in item.branches" :key="index">
                  {{ branch.branch_name
                  }}<span v-if="index !== item.branches.length - 1">, </span>
                </span>
              </template>
...
export default {
  data: () => ({
    ...
    snackColor: "",
    snackText: "",
    Model: "Branch Group Login",
    endpoint: "branch-group-login",
    search: "",
    snackbar: false,
    loading: false,
    actionLoading: false,
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.editedItem.branch_ids = this.editedItem.branches.map(e => e.id);
