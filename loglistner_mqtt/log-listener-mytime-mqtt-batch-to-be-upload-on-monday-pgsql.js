...
async function getDeviceInfo(deviceId) {
  ...
  if (!key) return { company_id: null, log_type: 'Auto' };
  ...
  try {
    const r = await dbPool.query(
      `SELECT company_id, function FROM devices WHERE serial_number = $1::text OR device_id = $1::text LIMIT 1`,
      [key]
    );
    const logTypeMap = { 'auto': 'Auto', 'In': 'In', 'Out': 'Out', 'option': 'Option' };
    const fn = r.rows[0]?.function ?? 'auto';
    const info = {
      company_id: r.rows[0]?.company_id ?? null,
      log_type: logTypeMap[fn] ?? 'Auto',
      ts: Date.now()
    };
    deviceCache.set(key, info);
    return info;
  } catch (err) {
    logError("getDeviceInfo failed (" + key + "): " + (err?.message || err));
    return { company_id: null, log_type: 'Auto' };
  }
}
...
function buildAttendanceBulkInsert(rows) {
  ...
  // Columns count = 14 (we include company_id and log_type now).
  const cols = 14;
  const params = [];
