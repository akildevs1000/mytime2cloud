...
export default {
  ...
  async created() {
    this.$axios.get(`/time_table/${this.$route.params.id}`).then(({ data }) => {
      this.payload = data;
      console.log(
        "🚀 ~ file: _id.vue ~ line 652 ~ this.$axios.get ~ data",
        data
      );
    });
  },
  methods: {
    ...
    store_schedule() {
      ...
      this.$axios
        .put(`/time_table/${this.$route.params.id}`, payload)
        .then(({ data }) => {
          this.loading = false;
          ...
          if (!data.status) {
            ...
            this.snackbar = true;
            this.response = "Time Slot updated successfully";
          }
        })
        .catch(({ message }) => {
          this.snackbar = true;
