...
console.log(`WebSocket Resource Monitor Server running on ws://${HOST}:${PORT}`);
console.log(`Monitoring processes with >50% CPU or Memory usage`);
console.log(`Updates every ${UPDATE_INTERVAL}ms`);
console.log('\nOpen resource-monitor-dashboard.html in your browser to view');
        ...
        if (ws.readyState === WebSocket.OPEN) {
            try {
                ...
                if (processes.length > 0) {
                    console.log(`Sent ${processes.length} high-resource processes to ${clientIp}`);
                }
            } catch (error) {
