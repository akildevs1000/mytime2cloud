...
class SplitShiftController extends Controller
{
    ...
    public function render($id, $date, $shift_type_id, $UserIds = [], $custom_render = false)
    {
        ...
        foreach ($employees as $row) {
            ...
            if (!count($data)) {
                $message = '<br/> ' . $message . $row->system_user_id . ' has No Logs to render';
                continue;
            }
            if (!$params["shift"]["id"]) {
                $message = '<br/> ' . $message . $row->system_user_id . ' : No shift configured on  date:' . $date;
                continue;
            }
        ...
        try {
            ...
            $message = "[" . $date . " " . date("H:i:s") .  "] Dual Shift. Log(s) have been rendered. Affected Ids: " . json_encode($UserIds) . '<br/>' . $message;
            Logger::channel("render_manual_logs")->info($message);


            return $message;
            return $this->getMeta("Dual Shift", $message);
        } catch (\Throwable $e) {
