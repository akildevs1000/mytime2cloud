    ...
    <v-row class="mt-5 mb-5">
      ...
      <v-col cols="6">
        <div class="text-right">
          <v-btn v-if="can(`leave_deleted`)" small color="error" class="mr-2 mb-2" @click="delteteSelectedRecords">
            Delete Selected Records
          </v-btn>
          <v-btn v-if="can(`leave_create`)" small color="primary" @click="dialog = true" class="mb-2">{{ Model }} +
          </v-btn>
        </div>
      </v-col>
    </v-row>
      ...
      <v-col md="">
        <v-data-table v-if="can(`leave_list_view`)" v-model="ids" show-select item-key="id" :headers="headers"
          ...
          <template v-slot:item.action="{ item }">
            ...
            <v-icon v-if="can(`leave_show`)" color="secondary" small class="mr-2" @click="viewItem(item)">
              mdi-eye
            </v-icon>
            <!-- <v-icon
  ...
  methods: {
    ...
    viewItem(item) {
      ...
      if (item.approved_by && item.is_approved == 1) {
        ...
        this.approved_by = item.approved_by.first_name;
        this.approved_status = "Reject By";
