      ...
      <v-col cols="12">
        <v-card elevation="2" class="pa-5">
          <v-row>
            ...
            <v-col md="6">
              <!-- {{ main_report_type }} -->
              <v-radio-group style="float: right" row v-model="main_report_type"
                :items="['General Report', 'Multi In/Out Report']" item-text="['General Report', 'Multi In/Out Report']">
                <v-radio @click="change_mani_report_type('')" label="Multi In/Out Report" value=""></v-radio>
                <v-radio checked="true" @click="change_mani_report_type('General Report')" label="General Report"
                  value="General Report"></v-radio>
              </v-radio-group>
              <!-- <v-col md="2">
            ...
            <v-col md="2" v-if="payload.report_type == 'Daily'">
              ...
              <div class="text-left mt-2">
                <v-menu class="mt-2" ref="daily_menu" v-model="daily_menu" :close-on-content-click="false"
                  ...
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field :hide-details="payload.daily_date" outlined dense v-model="payload.daily_date" readonly
              ...
              <div class="text-left">
                <v-menu ref="from_menu" v-model="from_menu" :close-on-content-click="false" :return-value.sync="from_date"
                  ...
                  <v-date-picker v-model="payload.from_date" no-title scrollable>
                    ...
                    <v-btn text color="primary" @click="set_date_save($refs.from_menu, payload.from_date)">
                      OK
                    </v-btn>
                  </v-date-picker>
                </v-menu>
              ...
              <div class="text-left">
                <v-menu ref="to_menu" v-model="to_menu" :close-on-content-click="false" :return-value.sync="to_date"
                  ...
                  <v-date-picker v-model="payload.to_date" :max="max_date" no-title scrollable>
                    ...
                    <v-btn text color="primary" @click="set_date_save($refs.to_menu, payload.to_date)">
                      OK
                    </v-btn>
                  </v-date-picker>
                </v-menu>
          ...
          <v-data-table dense v-if="can(`attendance_report_view`)" :headers="headers" :items="data" :loading="loading"
            ...
            <template v-slot:item.employee.department.name="{ item }">
              ...
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_department_name', $event)"
            ...
            <template v-slot:item.shift_type.name="{ item }">
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_shift_type_name', $event)"
                    v-model="datatable_search_textbox" label="Search Shift Name "></v-text-field>
                </template>
              </v-edit-dialog>
            ...
            <template v-slot:item.in="{ item }">
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                {{ item.in }}
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_time_in', $event)"
                    v-model="datatable_search_textbox" label="Search Time in "></v-text-field>
                </template>
              </v-edit-dialog>
            ...
            <template v-slot:item.out="{ item }">
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                {{ item.out }}
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_time_out', $event)"
                    v-model="datatable_search_textbox" label="Search Time out "></v-text-field>
                </template>
              </v-edit-dialog>
            ...
            <template v-slot:item.total_hrs="{ item }">
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                {{ item.total_hrs }}
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_total_hours', $event)"
                    v-model="datatable_search_textbox" label="Search Total Hours "></v-text-field>
                </template>
              </v-edit-dialog>
            ...
            <template v-slot:item.ot="{ item }">
              <v-edit-dialog large save-text="Reset" cancel-text="Ok" style="margin-left: 4%;" @save="getDataFromApi()"
                ...
                {{ item.ot }}
                <template v-slot:input>
                  <v-text-field @input="getDataFromApi_DatatablFilter('search_ot', $event)"
                    v-model="datatable_search_textbox" label="Search OT "></v-text-field>
                </template>
              </v-edit-dialog>
            ...
            <template v-slot:item.actions="{ item }" v-if="can('attendance_report_edit')">
