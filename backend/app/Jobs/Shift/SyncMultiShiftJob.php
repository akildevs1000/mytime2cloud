...
class SyncMultiShiftJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        Logger::channel('multi')->info('Queue: SyncMultiShiftJob Started', [
            'company_id' => $id,
            'date' => $date,
            'url' => $url,
        ]);
        ...
        $employeeIds->chunk(10)->each(function ($chunk) use ($id, $date, $url, $employeeIds) {
            ...
            Logger::channel('multi')->info('Queue request chunk', [
                'chunk' => $chunk->toArray(),
                'params' => $params,
            ]);
            ...
            try {
                ...
                if ($response->successful()) {
                    Logger::channel('multi')->info('Queue request successful', [
                        'chunk' => $chunk->toArray(),
                    ...
                    Logger::channel('multi')->error('Queue request failed', [
                ...
                Logger::channel('multi')->critical('Queue unexpected error', [
                    'exception' => $e->getMessage(),
                ]);
            }
        });

        Logger::channel('multi')->info('Queue: SyncMultiShiftJob Completed Successfully');
    }
}
