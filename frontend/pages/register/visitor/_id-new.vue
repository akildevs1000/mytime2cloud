<template>
  ...
  export default {
    ...
    async created() {
      ...
      if (/\btempSecret\b/.test(this.$route.params.id)) {
        const token = this.$route.params.id.split("-")[0];
  
        const decrypt = await Promise.resolve(this.$crypto.decrypt(token));
  
        params = decrypt.split("-");
      } else {
      ...
      this.payload.branch_id = params[2] || 0;
      this.payload.user_id = params[3] || 0;
      this.payload.number = params[4] || 0;
  
  
      if (this.payload.company_id > 0) {
        await this.getPurposes(this.payload.company_id);
      }
    },
