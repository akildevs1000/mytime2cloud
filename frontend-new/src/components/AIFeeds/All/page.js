    ...
    return (
        <>
            <div className="overflow-auto">
                <table className="w-full table-fixed text-left border-collapse min-w-[400px]">
                    <colgroup>
                    ...
                    <thead>
                        <tr className="bg-slate-100 dark:bg-slate-800 border-y border-slate-200 dark:border-slate-700">
                            ...
                            <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 whitespace-nowrap" style={{ width: '60%' }}>description</th>
                            <th className="px-2 py-3 text-xs text-right font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 whitespace-nowrap" style={{ width: '120px' }}>
                                <Input
                                    icon="search"
                                    type="text"
                                    placeholder="Search..."
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
                        {filteredRows.length > 0 ? filteredRows.map((row, index) => (
                            <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/70 transition-colors group relative">
                                ...
                                <td className="px-4 py-4 whitespace-nowrap text-xs text-slate-600 dark:text-slate-300"></td>
                            </tr>
                        )) : (
                            <tr>
