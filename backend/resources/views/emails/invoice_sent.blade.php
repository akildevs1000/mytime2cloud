                ...
                <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.06);">
                    <tr>
                        <td style="background:#7c3aed;padding:24px 28px;color:#ffffff;">
                            <div style="font-size:13px;letter-spacing:.4px;text-transform:uppercase;opacity:.8;">{{ config('app.name') }}</div>
                            <div style="font-size:22px;font-weight:700;margin-top:4px;">Invoice {{ $invoice->number }}</div>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px;">
                            ...
                            <p style="margin:0 0 14px 0;font-size:15px;">
                                Please find attached invoice <strong>{{ $invoice->number }}</strong>
                                dated <strong>{{ \Illuminate\Support\Carbon::parse($invoice->issue_date)->format('d M Y') }}</strong>
                                for <strong>{{ $invoice->currency }} {{ number_format((float) $invoice->total, 2) }}</strong>.
                            </p>
                            ...
                            @if (!empty($userMessage))
                                <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;margin:18px 0;">
                                    <tr>
                                        <td style="padding:14px 18px;font-size:14px;color:#374151;white-space:pre-wrap;">{{ $userMessage }}</td>
                                    </tr>
                                </table>
                            @endif
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;margin:18px 0;">
                                <tr>
                                    <td style="padding:14px 18px;font-size:14px;">
                                        <div style="color:#6b7280;font-size:12px;text-transform:uppercase;letter-spacing:.5px;">Amount Due</div>
                                        <div style="font-size:18px;font-weight:700;margin-top:4px;color:#111827;">
